...
describe("Ok", () => {
  ...
  describe("$andStack", () => {
    ...
    it("should not stack on to a non-stack array", () => {
      expect(
        Ok(["test"])
          .$andStack(() => Ok("test2"))
          .$andStack(() => Ok("test3"))
  ...
  describe("$andStackAsync", () => {
    it("should invoke the and function with the contained value", async () => {
      ...
      await Ok("test").$andStackAsync(fnAnd);

      expect(fnAnd).toHaveBeenCalledExactlyOnceWith("test");
    });

    it("should reject when the and function throws", async () => {
      await expect(Ok().$andStackAsync(fnThrow)).rejects.toBe(errThrow);
    });

    it("should reject when the and function rejects", async () => {
      await expect(Ok().$andStackAsync(fnReject)).rejects.toBe(errReject);
    });

    it("should resolve to the error when the and result is Err", async () => {
      await expect(
        Ok().$andStackAsync(() => Err("test")),
      ).resolves.toStrictEqual(Err("test"));

      await expect(
        Ok().$andStackAsync(async () => Err("test")),
      ).resolves.toStrictEqual(Err("test"));
    });
    ...
    it("should handle custom objects with the ResultLikeSymbol", async () => {
      await expect(
        Ok("test")
          .$andStackAsync(async () => ResultLikeOk)
          .$map((stack) => [...stack]),
      ).resolves.toStrictEqual(Ok(["test", "test"]));

      await expect(
        Ok("test").$andStackAsync(async () => ResultLikeErr),
      ).resolves.toStrictEqual(Err("test"));
    });
  });
